...
- [ ] G2 — add SceneDef.cleared field; scene-clear → render authored town.cleared beat + advance map node (SceneView pop true on allNpcsSolved; consume in quest_map._openTown & home._goToScene). files: hotspot.dart, scene_view.dart, quest_map_screen.dart, kotoba_home_screen.dart. items 48,50,53.
- [ ] H1 — Hotspot.hints: List<NazoHint>? (rung/textJa/coinCost) threaded through NazoScreen._hintLadder; generic fallback when null. HARD RAIL: no hint names the answer/eliminates a distractor (content-qa gate). Model+thread only; per-case copy is sequenced.

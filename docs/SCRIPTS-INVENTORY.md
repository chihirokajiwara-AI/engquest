...
| `smoke_flow.mjs` | #49 slice-3: **real-browser click-flow smoke** — loads the running app, enables the Flutter semantics tree (in-page click of `flt-semantics-placeholder`), CLICKS the core flow by semantic label, and asserts each navigation happened within a time budget. Measures true time-to-interactive + transition latency; flags DEAD taps (timeout), SLOW taps (>2.5s), and JS console/page errors | `node scripts/smoke_flow.mjs [baseUrl]` | The layer flutter_test can't cover: drives the ACTUAL CanvasKit app. Measured live :8088 boot ≈6.7s (SLOW) vs local fresh build ≈1s. Exit 1 on dead tap / JS error (SLOW is warn-only) |

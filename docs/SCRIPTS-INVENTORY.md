...
| `verify_audio_assets.py` | CI: confirms manifest `status=ready` MP3s exist byte-identical in both `assets/audio/a1/` and `web/audio/a1/`; also checks flat `web/audio/` | `python3 scripts/verify_audio_assets.py` | Run by pre-push gate and CI; currently FAILING (T35 open — audio dirs empty) |
| `test_audio_manifest.py` | CI: validates `audio_manifest.json` schema matches Dart `AudioManifestEntry.fromJson` contract; checks all 300 MP3s exist and are non-zero | `python3 scripts/test_audio_manifest.py` | Run by pre-push gate and CI; currently FAILING (T35) |

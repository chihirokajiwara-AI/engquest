...
| C16 | Parent Dashboard Real Data Wiring | `done` | 2026-05-29 | lib/core/analytics/firestore_progress_repository.dart — reads users/{uid}/profile + sessions/{dateISO}; recordSession() batch-writes session+profile atomically with merge (weighted avg score); calculateStreak() counts consecutive calendar days; getNextReviewDue() from cards subcollection; getLast7Days() parallel 7-doc fetch; graceful null fallback to mock on Firestore error; ProgressService upgraded with FirestoreProgressRepository injection (constructor param); BattleScreen calls _recordSessionToFirestore() on session complete (fire-and-forget, offline-safe); ParentDashboard shows LIVE/DEMO badge + refresh button; 25 tests (fake_cloud_firestore) |
| C17 | Level/XP System | `done` | 2026-05-29 | lib/core/gamification/xp_profile.dart — XpProfile model + levelFromXp() + kLevelThresholds (Lv.1-8) + kGradeXp (Again=0, Hard=5, Good=10, Easy=15); lib/core/gamification/xp_service.dart — XpService with Firestore persistence (users/{uid}/profile.totalXp), ValueNotifier<XpProfile?> for real-time UI, level-up detection (XpAwardResult.didLevelUp), awardXp(uid, grade) + awardXpBatch(), offline-safe (Firestore queue); BattleScreen wired: grade-aware XP popup (Again=0 → no popup), level-up celebratory dialog (レベルアップ！🎉 + progress bar, auto-dismiss 3s); WorldMapScreen wired: real XpProfile replaces mock constants, _XpLoadingSkeleton shown while loading, profileNotifier listener for live updates after battle; 34 tests (fake_cloud_firestore) |
| C18 | Word Audio Stubs + TTS Service | `done` | 2026-05-29 | assets/content/audio_manifest.json — 30 A1 seed words with IPA/syllableCount/storagePath/status; lib/core/audio/tts_service.dart — TtsService (manifest loader, local file cache, Google TTS REST API stub with full request body builder, batch prefetch, GoogleTtsConfig); lib/core/audio/word_audio_player_service.dart — WordAudioPlayerService (ChangeNotifier, state machine: idle/loading/playing/error, prefetchSession, WordAudioAutoPlay.trigger); scripts/generate_tts_audio.py — batch TTS generator (rate-limited 30req/min, updates manifest status pending→ready, gsutil upload instructions); pubspec: path_provider ^2.1.2 + equatable ^2.0.5 added; assets/audio/a1/ directory created; 26 tests |
| C19 | Firestore Security Rules + Deployment Config | `done` | 2026-05-29 | firestore.rules — production-grade security rules: user-isolated data (isOwner), field-level validation (FSRS card enum+range, XP/level bounds, session sanity), card ID injection prevention (regex), read-only vocabulary, default deny; firestore.indexes.json — 3 compound indexes for FSRS getDueCards + session queries; firebase.json — deploy config (firestore + hosting + emulators); .firebaserc — project alias engquest-mvp; Fix: app.dart named routes now pass OnboardingStorage.ageYears to BattleScreen/WorldMapScreen (was defaulting to 8 regardless of child age); test/firestore/firestore_rules_test.js — 30-case JS test suite (9 groups: unauth/cross-user/own-data/profile-validation/card-validation/session-validation/vocab/experiments/unknown-paths); Deploy: firebase deploy --only firestore:rules |
| P0.3 | nginx Production Web Serving (Hetzner VPS) | `done` | 2026-05-29 | deploy/nginx/engquest.conf — replaces throwaway `python -m http.server :8080`; gzip + gzip_static (precompressed .gz); immutable 1yr cache for hashed js/css/wasm; no-cache for index.html + sw.js + flutter_service_worker.js (instant deploys); 30d cache + CORS for mp3/svg/img/fonts; SPA fallback to index.html; server_tokens off; wasm MIME. deploy/install_nginx.sh — idempotent root installer: installs nginx, kills lingering python http.server on :8080, precompresses assets, deploys+symlinks config, removes default site, `nginx -t` validate before reload, `systemctl enable nginx` (survives reboot + crash auto-restart), post-deploy health check. deploy/test_nginx_config.py — 27 checks (config requirements + bash -n + sandboxed nginx -t), all green; live-verified in CI container: gzip ✅ immutable/no-cache headers ✅ CORS ✅ SPA fallback ✅. deploy/README.md — runbook. NOTE: install_nginx.sh must be run on the VPS over SSH (cannot deploy from CI); :8080 was DOWN at task time and this fixes it permanently. |
...
| S04 | firebase-offline | Does Firebase Firestore offline mode handle FSRS scheduling without connectivity? | ✅ COMPLETE 2026-05-29 — YES: persistenceEnabled=true + CACHE_SIZE_UNLIMITED works. Cards graded offline persist across restarts. getDueCards() reads from local cache. First-launch offline falls back to InMemory. LWW conflict accepted for single-device MVP. See src/spikes/firebase-offline/README.md |

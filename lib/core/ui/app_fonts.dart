...
///
/// A single variable TTF (`assets/fonts/NotoSerifJP.ttf`, family declared in
/// pubspec.yaml) carries every weight; [TextStyle.fontWeight] drives the `wght`
/// axis, so all former google_fonts weight requests (w300–w900) are served from
/// the one file. This also removes the runtime font download from the cold-boot
/// path (helps the load-speed P0).
///
/// IMPORTANT: the bundled file is a ~2 MB SUBSET (not the full ~13 MB Noto Serif
/// JP), generated by `scripts/subset_jp_font.py` from every character used in
/// `lib/**.dart` + `assets/data` + `assets/content`. New Japanese text is safe to
/// add — the subset is coverage-gated by `scripts/verify_font_coverage.py` (CI +
/// pre-push), so an uncovered glyph FAILS the gate instead of shipping silent
/// tofu (□); rerun `subset_jp_font.py` to re-include any newly-added character.
